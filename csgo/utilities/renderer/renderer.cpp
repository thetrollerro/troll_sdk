...
void render::create_font( void** font, const std::string& family, int size, bool bold ) {
	...
	D3DXCreateFontA( interfaces::directx, size, 0, bold ? FW_BOLD : FW_LIGHT, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, 0, DEFAULT_PITCH | FF_DONTCARE, family.data( ), &d3d_font );
	*font = d3d_font;
}

void render::create_font( void** font, const std::string& family, int size, int width ) {
	ID3DXFont* d3d_font = nullptr;
	D3DXCreateFontA( interfaces::directx, size, 0, width, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, 0, DEFAULT_PITCH | FF_DONTCARE, family.data( ), &d3d_font );
	*font = d3d_font;
}
...
void render::rectangle( int x, int y, int width, int height, std::uint32_t color ) {
	...
	interfaces::directx->SetRenderState( D3DRS_ALPHABLENDENABLE, true );
	interfaces::directx->SetTexture( 0, nullptr );
	interfaces::directx->SetPixelShader( nullptr );
	interfaces::directx->SetRenderState( D3DRS_SRCBLEND, D3DBLEND_SRCALPHA );
	interfaces::directx->SetRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
	interfaces::directx->SetFVF( D3DFVF_XYZRHW | D3DFVF_DIFFUSE );
	interfaces::directx->DrawPrimitiveUP( D3DPT_TRIANGLESTRIP, 2, &vert, sizeof( vtx_t ) );
}
...
void render::line( int x, int y, int x2, int y2, std::uint32_t color ) {
	...
	interfaces::directx->SetRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
	interfaces::directx->SetFVF( D3DFVF_XYZRHW | D3DFVF_DIFFUSE );
	interfaces::directx->DrawPrimitiveUP( D3DPT_LINELIST, 1, &vert, sizeof( vtx_t ) );
}
...
void render::circle( int x, int y, int radius, int segments, std::uint32_t color ) {
	...
	interfaces::directx->CreateVertexBuffer( ( segments + 2 ) * sizeof( vtx_t ), D3DUSAGE_WRITEONLY, D3DFVF_XYZRHW | D3DFVF_DIFFUSE, D3DPOOL_DEFAULT, &vb, nullptr );
	...
	interfaces::directx->SetRenderState( D3DRS_SRCBLEND, D3DBLEND_SRCALPHA );
	interfaces::directx->SetRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
	interfaces::directx->SetStreamSource( 0, vb, 0, sizeof( vtx_t ) );
	interfaces::directx->SetFVF( D3DFVF_XYZRHW | D3DFVF_DIFFUSE );
	interfaces::directx->DrawPrimitive( D3DPT_TRIANGLEFAN, 0, segments );
...
void render::texture( unsigned char* data, int x, int y, int width, int height, float scale ) {
	...
	D3DXCreateSprite( interfaces::directx, &sprite );

	IDirect3DTexture9* tex = nullptr;
	D3DXCreateTextureFromFileInMemory( interfaces::directx, data, 4 * width * height, &tex );

	interfaces::directx->SetRenderState( D3DRS_ALPHABLENDENABLE, true );
	interfaces::directx->SetRenderState( D3DRS_SRCBLEND, D3DBLEND_SRCALPHA );
	interfaces::directx->SetRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
	interfaces::directx->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_SELECTARG1 );
	interfaces::directx->SetTextureStageState( 0, D3DTSS_COLORARG1, D3DTA_TEXTURE );
	interfaces::directx->SetTextureStageState( 0, D3DTSS_COLORARG2, D3DTA_DIFFUSE );
	interfaces::directx->SetFVF( D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1 );
	interfaces::directx->SetPixelShader( nullptr );
...
void render::clip_rect( int x, int y, int width, int height ) {
	...
	interfaces::directx->SetScissorRect( &rect );
}
