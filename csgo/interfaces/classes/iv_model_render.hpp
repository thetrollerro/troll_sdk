...
public:
	...
	virtual void force_mat( i_material* mat, int override_type = 0, int nOverrides = 0 ) = 0;
	virtual bool is_forced_mat( void ) = 0;
	virtual void set_view_target( const studio_hdr_t* studio_hdr, int body_index, const vec3_t& target ) = 0;
	...
	virtual bool draw_model_setup( model_render_info_t& info, draw_model_state_t* state, matrix_t* bone_to_world_out ) = 0;
	virtual void draw_model_execute( void* ctx, draw_model_state_t& state, const model_render_info_t& info, matrix_t* custom_bone_to_world = nullptr ) = 0;
